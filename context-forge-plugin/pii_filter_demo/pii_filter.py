...
try:
    ...
    if _RUST_AVAILABLE:
        logger.info(
            "🦀 Rust PII filter available - using high-performance implementation (5-100x speedup)"
        )
    else:
        logger.info(
            "Rust module found but RUST_AVAILABLE=False - using Python implementation"
        )
...
class PIIFilterConfig(BaseModel):
    ...
    detect_bsn: bool = Field(
        default=True, description="Detect Dutch BSN (Burgerservicenummer)"
    )
    detect_credit_card: bool = Field(
    ...
    detect_date_of_birth: bool = Field(
        default=True, description="Detect dates of birth"
    )
    detect_passport: bool = Field(default=True, description="Detect passport numbers")
    detect_driver_license: bool = Field(
        default=True, description="Detect driver's license numbers"
    )
    detect_bank_account: bool = Field(
        default=True, description="Detect bank account numbers"
    )
    detect_medical_record: bool = Field(
        default=True, description="Detect medical record numbers"
    )
    ...
    default_mask_strategy: MaskingStrategy = Field(
        default=MaskingStrategy.REDACT, description="Default masking strategy"
    )
    redaction_text: str = Field(
        default="[REDACTED]", description="Text to use for redaction"
    )

    # Behavior configuration
    block_on_detection: bool = Field(
        default=False, description="Block request if PII is detected"
    )
    log_detections: bool = Field(default=True, description="Log PII detections")
    include_detection_details: bool = Field(
        default=True, description="Include detection details in metadata"
    )

    # Custom patterns
    custom_patterns: List[PIIPattern] = Field(
        default_factory=list, description="Custom PII patterns to detect"
    )

    # Whitelist configuration
    whitelist_patterns: List[str] = Field(
        default_factory=list, description="Patterns to exclude from PII detection"
    )
...
class PIIDetector:
        ...
        if self.config.detect_bank_account:
            patterns.extend(
                [
                    ...
                    PIIPattern(
                        type=PIIType.BANK_ACCOUNT,
                        pattern=r"\b[A-Z]{2}\d{2}[A-Z0-9]{4}\d{7}(?:\d{3})?\b",
                        description="IBAN",
                        mask_strategy=MaskingStrategy.PARTIAL,
                    ),  # IBAN
                ]
                    ...
                    if not self._is_whitelisted(text, match.start(), match.end()):
                        ...
                        for start, end in seen_ranges:
                            if (
                                ...
                                or (match.end() > start and match.end() <= end)
                                or (match.start() <= start and match.end() >= end)
                            ):
                                overlaps = True
                                break
    ...
    def mask(self, text: str, detections: Dict[PIIType, List[Dict]]) -> str:
        ...
        for detection in all_detections:
            ...
            masked_value = self._apply_mask(
                detection["value"], detection["type"], strategy
            )
            masked_text = (
    ...
    def _apply_mask(
        self, value: str, pii_type: PIIType, strategy: MaskingStrategy
    ) -> str:
...
class PIIFilterPluginDemo(Plugin):
    ...
    def __init__(self, config: PluginConfig):
        ...
        if _RUST_AVAILABLE and _RustPIIDetector is not None:
            ...
            logger.info(
                "🦀 PIIFilterPluginDemo initialized with Rust acceleration (5-100x speedup)"
            )
        else:
            self.detector = PIIDetector(self.pii_config)
            self.implementation = "Python"
            logger.info("🐍 PIIFilterPluginDemo initialized with Python implementation")
            ...
            if isinstance(value, str):
                ...
                if detections:
                    ...
                    if self.pii_config.log_detections:
                        logger.warning(
                            f"PII detected in prompt argument '{key}': {', '.join(detections.keys())}"
                        )
                    ...
                    if self.pii_config.block_on_detection:
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                f"PII filter blocking prompt - full content: {value}"
                            )
                        else:
                            # Log at WARNING level with redacted content
                            logger.warning(
                                f"PII filter blocked prompt in argument '{key}' - detected types: {detected_types} (content redacted)"
                            )
                        violation = PluginViolation(
                            ...
                            details={
                                ...
                                "count": sum(
                                    len(items) for items in detections.values()
                                ),
                            },
                        )
                        return PromptPrehookResult(
        ...
        if all_detections:
            return PromptPrehookResult(
                modified_payload=PromptPrehookPayload(
                    prompt_id=payload.prompt_id, args=modified_args
                )
            )
        ...
        for message in payload.result.messages:
            if message.content and hasattr(message.content, "text"):
                ...
                if detections:
                    ...
                    if self.pii_config.log_detections:
                        logger.warning(
                            f"PII detected in {message.role} message: {', '.join(detections.keys())}"
                        )
    ...
    async def tool_pre_invoke(
        ...
        """
        logger.debug(
            f"Processing tool pre-invoke for tool '{payload.name}' with {len(payload.args) if payload.args else 0} arguments"
        )
        ...
        modified, detections = self._process_nested_data_for_pii(
            payload.args, "args", all_detections
        )
        ...
        if detections and self.pii_config.block_on_detection:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"PII filter blocking tool '{payload.name}' - full arguments: {payload.args}"
                )
            else:
                # Log at WARNING level with redacted content
                logger.warning(
                    f"PII filter blocked tool '{payload.name}' arguments - detected types: {detected_type_list} (content redacted)"
                )
            violation = PluginViolation(
    ...
    async def tool_post_invoke(
        ...
        """
        logger.debug(
            f"Processing tool post-invoke for tool '{payload.name}', result type: {type(payload.result).__name__}"
        )
        ...
        if isinstance(payload.result, str):
            ...
            if detections:
                ...
                if self.pii_config.log_detections:
                    logger.warning(
                        f"PII detected in tool result: {', '.join(detections.keys())}"
                    )
                ...
                if self.pii_config.block_on_detection:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(
                            f"PII filter blocking tool '{payload.name}' result - full content: {payload.result}"
                        )
                    else:
                        # Log at WARNING level with redacted content
                        logger.warning(
                            f"PII filter blocked tool '{payload.name}' result - detected types: {detected_types} (content redacted)"
                        )
                    violation = PluginViolation(
            ...
            modified, detections = self._process_nested_data_for_pii(
                payload.result, "result", all_detections
            )
            if detections and self.pii_config.block_on_detection:
                detected_types = list(
                    set(
    ...
    def _process_nested_data_for_pii(
        self, data: Any, path: str, all_detections: dict
    ) -> tuple[bool, bool]:
        """
        ...
        if isinstance(data, str):
            ...
            if detections:
                ...
                if self.pii_config.log_detections:
                    logger.warning(
                        f"PII detected in tool result at '{path}': {', '.join(detections.keys())}"
                    )

            # Try to parse as JSON and process nested content
            try:
                parsed_json = orjson.loads(data)
                json_modified, json_detections = self._process_nested_data_for_pii(
                    parsed_json, f"{path}(json)", all_detections
                )
            ...
            for key, value in data.items():
                ...
                value_modified, value_detections = self._process_nested_data_for_pii(
                    value, current_path, all_detections
                )
                ...
                if value_modified and isinstance(value, str):
                    ...
                    if any(
                        path.startswith(json_path) for path in all_detections.keys()
                    ):
                        try:
                            parsed_json = orjson.loads(value)
                            # Apply masking to the parsed JSON
                            self._apply_pii_masking_to_parsed_json(
                                parsed_json, json_path, all_detections
                            )
            ...
            for i, item in enumerate(data):
                ...
                item_modified, item_detections = self._process_nested_data_for_pii(
                    item, current_path, all_detections
                )
    ...
    def _apply_pii_masking_to_parsed_json(
        self, data: Any, base_path: str, all_detections: dict
    ) -> None:
        """
    ...
    async def shutdown(self) -> None:
        ...
        logger.info(
            f"PII Filter plugin ({self.implementation}) shutting down. Total masked: {self.masked_count} items"
        )
